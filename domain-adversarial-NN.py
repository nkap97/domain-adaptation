...
class Path(object):
    ...
    def db_dir(database):
        if database == 'source':
            # folder that contains class labels, # Save preprocess data into output_dir
    ...
    @staticmethod
    def model_dir():
        return '/content/drive/My Drive/pytorch-video-recognition-master/c3d-pretrained.pth'
...
class VideoDataset(Dataset):
    def __init__(self, dataset='source', split='train', clip_len=16, preprocess=False):
        ...
        if dataset == "source":
            if not os.path.exists('/content/drive/My Drive/pytorch-video-recognition-master/dataloaders/ucf_labels.txt'):
                with open('/content/drive/My Drive/pytorch-video-recognition-master/dataloaders/ucf_labels.txt', 'w') as f:
...
#     #img_files = os.listdir('/content/drive/My Drive/images')
#     #inputs = cv2.imread(img_files)
#     #inputs = [cv2.imread(img_file) for img_file in glob.glob("/content/drive/My Drive/images/*.png")
#     #images = [cv2.imread(file) for file in glob.glob("path/to/files/*.png")]
#     #inputs, classes = next(iter(dataloaders['train']))
#     net = C3D(num_classes=50, pretrained=True)

#     #outputs = net.forward(inputs)
#     c3d_feature_outputs = net.forward(loader_input)
#     print(c3d_feature_outputs)
#     print(c3d_feature_outputs.size())
...
path1 = torch.load('/content/drive/My Drive/Results/s_class_list.pth')
path2 = torch.load('/content/drive/My Drive/Results/s_domain_list.pth')
path3 = torch.load('/content/drive/My Drive/Results/t_domain_list.pth')
...
plt.plot(path1)
plt.xlabel('No. of Epochs')
plt.ylabel('Label Predictor Error')

plt.plot(path2)
plt.xlabel('No. of Epochs')
plt.ylabel('Class Domain Error')
